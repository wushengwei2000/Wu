...
using Orchard.Comments.Projections;
...
namespace Orchard.Comments.Services {
    ...
    public class CommentService : ICommentService {
        ...
        private readonly HashSet<string> _processedCommentsPartsUriID = new HashSet<string>();
        ...
        public ContentItem GetCommentedContent(int id) {
            ...
            if (result == null)
                result = _orchardServices.ContentManager.Get(id, VersionOptions.AllVersions);   
            return result;
        }
        ...
        //New Commnet Implementation based on Routing 

        public IContentQuery<CommentPart, CommentPartRecord> GetComments(string url)
        {
            return GetComments()
                .Where(c => c.CommentedOnUrl==url);
        }

        public IContentQuery<CommentPart, CommentPartRecord> GetCommentsForUrl(CommentStatus status, string url)
        {
            return GetComments()
                .Where(c => c.Status == status && c.CommentedOnUrl==url);
        }


        public IContentQuery<CommentPart, CommentPartRecord> GetCommentsForCommentedContent(int id, string url)
        {
            return GetComments()
                .Where(c => c.CommentedOn == id && url==c.CommentedOnUrl);
        }
        ...
        public void ProcessCommentsCount(int commentsPartId, string url)
        {
            var key = string.Format("{0}_{1}", commentsPartId, url.ToLower());
            if (!_processedCommentsPartsUriID.Contains(key))
            {
                _processedCommentsPartsUriID.Add(key);
                _processingEngine.AddTask(_shellSettings, _shellDescriptorManager.GetShellDescriptor(), "ICommentsCountProcessor.Process", new Dictionary<string, object> { { "commentsPartId", commentsPartId }, {"url", url} });
            }
        }
    }
}
