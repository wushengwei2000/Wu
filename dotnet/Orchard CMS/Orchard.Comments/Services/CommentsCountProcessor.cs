...
namespace Orchard.Comments.Services {
    public class CommentsCountProcessor : ICommentsCountProcessor {
        ...
        public void Process(int commentsPartId, string url)
        {
            var commentsCount = _contentManager
                .Query<CommentPart, CommentPartRecord>()
                .Where(x => x.Status == CommentStatus.Approved && x.CommentedOn == commentsPartId && x.CommentedOnUrl.IsSamePageAs(url))
                .Count();
